...
import { AutorizarAnticipoButton } from "../../anticipos/anticipo-forms";

export default async function GerenciaAnticiposPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Anticipos a autorizar</h1>
        <p className="text-slate-500">Solicitudes de anticipo que superan el umbral configurado.</p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {anticipos.map((a) => (
          <div key={a.id} className="rounded-lg border border-slate-200 bg-white p-5">
            ...
            <p className="mb-3 text-sm text-slate-700">{formatMoneda(a.monto)}</p>
            <AutorizarAnticipoButton anticipoId={a.id} />
          </div>
        ))}
        {anticipos.length === 0 && (
          <p className="rounded-lg border border-slate-200 bg-white p-6 text-center text-slate-400">
            No hay anticipos pendientes de autorización.
          </p>
        )}
      </div>
    </div>
